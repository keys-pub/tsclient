...
export class KeysService extends EventEmitter {
  ...
  ChannelInvitesCreate(req: keys.ChannelInvitesCreateRequest): Promise<keys.ChannelInvitesCreateResponse> {
    return new Promise<keys.ChannelInvitesCreateResponse>((resolve, reject) => {
	  this.log.info('ChannelInvitesCreate')
      this.service().ChannelInvitesCreate(req, (err: RPCError, resp: keys.ChannelInvitesCreateResponse) => {
        if (err) {
          reject(err)
          this.emitError(err)
          return

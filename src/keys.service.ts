...
// InputID: 82214873b6f53b2eefb3ae7234a6d569bedd962d
...
export class KeysService extends EventEmitter {
  ...
  ChannelInviteCreate(req: keys.ChannelInviteCreateRequest): Promise<keys.ChannelInviteCreateResponse> {
    return new Promise<keys.ChannelInviteCreateResponse>((resolve, reject) => {
      this.service().ChannelInviteCreate(req, (err: RPCError, resp: keys.ChannelInviteCreateResponse) => {
        if (err) {
          reject(err)
          this.emitError(err)
          return
  ...
  ChannelInviteAccept(req: keys.ChannelInviteAcceptRequest): Promise<keys.ChannelInviteAcceptResponse> {
    return new Promise<keys.ChannelInviteAcceptResponse>((resolve, reject) => {
      this.service().ChannelInviteAccept(req, (err: RPCError, resp: keys.ChannelInviteAcceptResponse) => {
        if (err) {
          reject(err)
          this.emitError(err)
          return
